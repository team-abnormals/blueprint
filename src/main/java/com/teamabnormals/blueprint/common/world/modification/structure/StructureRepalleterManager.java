...
public final class StructureRepalleterManager extends SimpleJsonResourceReloadListener {
	 ...
	 * @param random   A {@link Random} instance to use for the repaletters.
	 */
	public static void update(ResourceLocation location, Random random) {
	...
	public static BlockState getBlockState(ServerLevelAccessor level, BlockState state) {
		...
		Random random = activeData.random;
	...
	protected void apply(Map<ResourceLocation, JsonElement> map, ResourceManager manager, ProfilerFiller profilerFiller) {
		...
		HashMap<ResourceLocation, ArrayList<StructureRepaletterEntry>> unassignedMap = new HashMap<>();
		int count = 0;
		for (var mapEntry : map.entrySet()) {
			...
			try {
				...
				entry.selector().select(configuredStructureFeatureLocations::forEach).forEach(location -> {
					unassignedMap.computeIfAbsent(location, __ -> new ArrayList<>()).add(entry);
				});
				count++;
		...
		unassignedMap.forEach((location, repaletters) -> {
