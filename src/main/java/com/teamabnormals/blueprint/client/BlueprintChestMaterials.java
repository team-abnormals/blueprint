	...
	public static synchronized String registerMaterials(String modId, String name, boolean trapped) {
		...
		if (FMLEnvironment.dist == Dist.CLIENT) {
			MATERIALS.put(registryName, Suppliers.memoize(() -> {
				Material single = new Material(Sheets.CHEST_SHEET, ResourceLocation.fromNamespaceAndPath(modId, "entity/chest/" + name + "/" + chestType));
				Material left = new Material(Sheets.CHEST_SHEET, ResourceLocation.fromNamespaceAndPath(modId, "entity/chest/" + name + "/" + chestType + "_left"));
				Material right = new Material(Sheets.CHEST_SHEET, ResourceLocation.fromNamespaceAndPath(modId, "entity/chest/" + name + "/" + chestType + "_right"));
