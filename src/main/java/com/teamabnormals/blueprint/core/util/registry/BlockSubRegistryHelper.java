...
public class BlockSubRegistryHelper extends AbstractSubRegistryHelper<Block> {
	...
	public Pair<RegistryObject<BlueprintStandingSignBlock>, RegistryObject<BlueprintWallSignBlock>> createSignBlock(String name, WoodType woodType, MapColor color) {
		RegistryObject<BlueprintStandingSignBlock> standing = this.deferredRegister.register(name + "_sign", () -> new BlueprintStandingSignBlock(Block.Properties.of().mapColor(color).ignitedByLava().noCollission().strength(1.0F).sound(SoundType.WOOD), woodType));
		RegistryObject<BlueprintWallSignBlock> wall = this.deferredRegister.register(name + "_wall_sign", () -> new BlueprintWallSignBlock(Block.Properties.of().mapColor(color).ignitedByLava().noCollission().strength(1.0F).sound(SoundType.WOOD).dropsLike(standing.get()), woodType));
