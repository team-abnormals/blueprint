...
public class SlabfishHatScreen extends Screen {
	...
	protected void init() {
		...
		for (RewardHandler.SlabfishSetting setting : RewardHandler.SlabfishSetting.values()) {
			...
			}, (button, stack, mouseX, mouseY) -> this.renderTooltip(stack, this.font.func_238425_b_(new TranslationTextComponent("abnormals_core.config.slabfish_hat." + setting.name().toLowerCase(Locale.ROOT) + ".tooltip"), 200), mouseX, mouseY)));
			++i;
		}
		...
		this.addButton(new Button(this.width / 2 - 100, this.height / 6 + 24 * (i >> 1), 200, 20, DialogTexts.field_240632_c_, (button) -> this.getMinecraft().displayGuiScreen(this.parent)));
	}
	...
	private ITextComponent getOptionName(RewardHandler.SlabfishSetting setting, boolean enabled) {
		return new TranslationTextComponent("abnormals_core.config.slabfish_hat." + setting.name().toLowerCase(Locale.ROOT)).appendString(": ").append(DialogTexts.func_240638_a_(enabled));
	}
}