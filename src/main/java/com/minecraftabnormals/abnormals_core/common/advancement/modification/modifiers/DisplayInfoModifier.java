...
public final class DisplayInfoModifier extends AdvancementModifier<DisplayInfoModifier.Config> {
	...
	public DisplayInfoModifier() {
		super(((element, conditionArrayParser) -> {
			...
			Optional<ITextComponent> title = JSONUtils.hasField(object, "title") ? Optional.ofNullable(ITextComponent.Serializer.getComponentFromJson(object.get("title"))) : Optional.empty();
			Optional<ITextComponent> description = JSONUtils.hasField(object, "description") ? Optional.ofNullable(ITextComponent.Serializer.getComponentFromJson(object.get("description"))) : Optional.empty();
			Optional<ItemStack> icon = JSONUtils.hasField(object, "icon") ? Optional.of(deserializeIcon(JSONUtils.getJsonObject(object, "icon"))) : Optional.empty();
			Optional<ResourceLocation> background = JSONUtils.hasField(object, "background") ? Optional.of(new ResourceLocation(JSONUtils.getString(object, "background"))) : Optional.empty();
			Optional<FrameType> frameType = JSONUtils.hasField(object, "frame") ? Optional.of(FrameType.byName(JSONUtils.getString(object, "frame"))) : Optional.empty();
			Optional<Boolean> showToast = JSONUtils.hasField(object, "show_toast") ? Optional.of(JSONUtils.getBoolean(object, "show_toast")) : Optional.empty();
			Optional<Boolean> announceToChat = JSONUtils.hasField(object, "announce_to_chat") ? Optional.of(JSONUtils.getBoolean(object, "announce_to_chat")) : Optional.empty();
			Optional<Boolean> hidden = JSONUtils.hasField(object, "hidden") ? Optional.of(JSONUtils.getBoolean(object, "hidden")) : Optional.empty();
	...
	private static ItemStack deserializeIcon(JsonObject object) {
		if (!object.has("item")) {
			...
			if (object.has("data")) {
				...
				if (object.has("nbt")) {
					try {
						CompoundNBT nbt = JsonToNBT.getTagFromJson(JSONUtils.getString(object.get("nbt"), "nbt"));
						stack.setTag(nbt);
