...
public final class ACLayerUtil {
	...
	public static <R extends IExtendedNoiseRandom<LazyArea>> Layer createEndBiomeLayer(Registry<Biome> lookupRegistry, LongFunction<R> contextFactory) {
		IAreaFactory<LazyArea> biomesFactory = new EndBiomesLayer(lookupRegistry).apply(contextFactory.apply(1L));
	...
	public enum VoroniZoomLayer implements IAreaTransformer1 {
		...
		public int apply(IExtendedNoiseRandom<?> extendedNoiseRandom, IArea area, int p_215728_3_, int p_215728_4_) {
			int i = p_215728_3_ - 2;
			int j = p_215728_4_ - 2;
			int k = i >> 2;
			int l = j >> 2;
			int i1 = k << 2;
			int j1 = l << 2;
			extendedNoiseRandom.setPosition(i1, j1);
			double d0 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D;
			double d1 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D;
			extendedNoiseRandom.setPosition(i1 + 4, j1);
			double d2 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D + 4.0D;
			double d3 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D;
			extendedNoiseRandom.setPosition(i1, j1 + 4);
			double d4 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D;
			double d5 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D + 4.0D;
			extendedNoiseRandom.setPosition(i1 + 4, j1 + 4);
			double d6 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D + 4.0D;
			double d7 = ((double) extendedNoiseRandom.random(1024) / 1024.0D - 0.5D) * 3.6D + 4.0D;
			int k1 = i & 3;
			int l1 = j & 3;
	...
	static class EndBiomesLayer implements IAreaTransformer0 {
		...
		public int apply(INoiseRandom random, int x, int z) {
			return this.lookupRegistry.getId(this.lookupRegistry.getValueForKey(BiomeUtil.getEndBiome(random)));
		}
	}
}
